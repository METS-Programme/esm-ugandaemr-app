...
type Props = {
  ...
  onChange: (next: ProgramReference[]) => void;
  disabled?: boolean;
};

function itemToString(p: ProgramReference | null) {
  if (!p) return '';
  return p.display;
}

export default function ProgramSearchMultiSelect({
  id,
  labelText,
  ...
  const { loading, results, error } = useProgramSearch(query);

  const addSelected = React.useCallback(
    (picked: ProgramReference | null) => {
      if (!picked) return;

      const next = new Map<string, ProgramReference>();
